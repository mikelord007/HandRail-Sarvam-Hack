...
/**
 * Design tokens ported verbatim from the "Classical" design system
 * (HandRailUI/design_handoff_handrail_app/_ds/classical-…/styles.css).
 *
 * This is an editorial, paper-ground, stroke-first palette — NOT Material.
 * Gold (`Accent` and its scale) is used as a stroke/text color everywhere
 * in the product except one place: the hand-back confirm button is the
 * single solid-gold fill in the whole app, and that contrast is deliberate
 * — see HandbackScreen. Do not fill anything else with Accent.
 */
object HandrailColors {
    // Ground
