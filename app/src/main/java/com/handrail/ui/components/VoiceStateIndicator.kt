...
/**
 * The mark + its word, together — per the design, "a symbol alone is not
 * accessible to this audience," so there is no variant that renders only
 * the animated mark. [onDark] selects the overlay/promise palette (gold at
 * accent-300, idle ring at 40% white) vs. the light-ground palette.
 */
@Composable
...
private fun VoiceStateMark(state: VoiceState, size: Dp, onDark: Boolean) {
    ...
    val ringColor = if (onDark) HandrailColors.Accent300 else HandrailColors.Accent
    val idleColor = if (onDark) HandrailColors.OnDarkInk40 else HandrailColors.Neutral400
    val barColor = if (onDark) HandrailColors.Accent300 else HandrailColors.Accent700
    val thinkingBaseColor = if (onDark) HandrailColors.OnDarkRuleStrong else HandrailColors.Neutral300
