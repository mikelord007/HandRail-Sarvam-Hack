...
/**
 * Maps a Sarvam language code (e.g. "hi-IN", "od-IN" — see [com.handrail.speech.SupportedLanguages])
 * to the Android [Locale] that resolves the matching `values-<qualifier>` resources.
 * Odia's ISO 639-1 code is "or", not Sarvam's "od", so it needs remapping;
 * every other Sarvam code's language part already matches its Android qualifier.
 */
 ...
 * string tied to a specific language from plain (non-Composable) code — e.g. the TTS
 * sample line in onboarding, where the string must match the language just picked, not
 * whatever the Activity happened to launch with.
 */
 ...
 * onboarding nav state (see MainActivity.onCreate's onboarding-complete check),
 * so the whole app is switched by re-pointing the composition's Context instead.
 */
@Composable
