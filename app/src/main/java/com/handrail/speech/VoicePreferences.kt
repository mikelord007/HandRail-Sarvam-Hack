...
/**
 * Persists everything about how Handrail speaks and listens: language,
 * speaker, "Speak slowly", and "Narrate every step". Replaces the old
 * LanguagePreferences — same prefs file and the same "language_code" key,
 * so an existing install's chosen language survives the upgrade.
 *
 * "Always hand back before payment" is deliberately NOT here: it is
 * hard-coded true in the agent loop's irreversible-action guard, never a
 * user preference, per CLAUDE.md.
 */
class VoicePreferences(context: Context) {
    ...
    /** Convenience snapshot for callers that just want to hand VoiceSettings to the agent loop. */
